...
def send_to_powerbi_endpoint():
    ...
    if isinstance(dashboard_view_lalin_daily_data, list) and dashboard_view_lalin_daily_data:
        ...
        if last_value is not None:
            difference = new_value - float(last_value)
            if difference < 0:  # Jika selisih negatif, kirimkan 0
                difference = 0
        ...
        else:  # Kasus pertama kali dijalankan
            formatted_data = [{"volume_lalin": new_value, "date": datetime.now().strftime('%d-%b-%y')}]
            print("Format Data: ", formatted_data)
            send_to_powerbi(formatted_data)
