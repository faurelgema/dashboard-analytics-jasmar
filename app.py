    ...
    try:
        ...
        if isinstance(dashboard_view_lalin_daily_data, list) and dashboard_view_lalin_daily_data:
            ...
            if current_date != last_date or last_date is None:
                ...
                return jsonify({'status': 'Data successfully sent to Power BI', 'volume_lalin_before': 0, 'volume_lalin_after': current_volume, 'difference': difference, 'date': current_date.strftime('%d-%b-%Y')})
                ...
                return jsonify({'status': 'Data successfully sent to Power BI', 'volume_lalin_before': last_volume, 'volume_lalin_after': current_volume, 'difference': difference, 'date': current_date.strftime('%d-%b-%Y')})
